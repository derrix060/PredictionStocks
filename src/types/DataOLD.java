...
public class DataOLD {
	...
	public DataOLD(String ticker, Calendar from, Calendar to, float margin, double maxValue, double minValue, double maxNormalizedValue, double minNormalizedValue, boolean isIdealOutput) throws IOException{
		setFrom(from);
		setTo(to);
		setMargin(margin);

		
		HashSet<Data> tickers = YahooExtractor.getHistorical(ticker, from, to);
	...
	public DataOLD(String ticker, Calendar from, Calendar to, float margin) throws IOException{
		this(ticker, from, to, margin, 0, 0, 0, 0, false);
	}
	
	private double[][] tickerToDouble (Set<Data> tickers){
